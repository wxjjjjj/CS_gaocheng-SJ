...
/* 可以在此定义其它需要的函数 */
void chushihua(struct tj_time* result)
{
	result->tj_year = 1970;
	result->tj_month = 1;
	result->tj_day = 1;
	result->tj_hour = 8;    //东八区，差8h
	result->tj_minute = 0;
	result->tj_second = 0;
	return;
}
...
void system_time_output(const time_t input_time)  //time_t的本质是64位无符号整数
{
    ...
    tt = localtime(&input_time);	//localtime为系统函数                                      !! localtime 函数将UTC时间转换为了本地时间。中国在东八区，故会差8小时！！
...
/***************************************************************************
  ...
  说    明：
***************************************************************************/
struct tj_time *tj_time_convert(int input_time) //(year % 4 == 0) && (year % 100 != 0) || (year % 400 == 0)   //真，则为闰年,二月有29天
{
	...
	if (result.tj_minute >= 60) {
		result.tj_hour = result.tj_hour+result.tj_minute / 60;
		result.tj_minute = result.tj_minute %60;
	}
	if (result.tj_hour >= 24) {
		result.tj_day = result.tj_day+result.tj_hour / 24;
		result.tj_hour = result.tj_hour % 24;
	}
	if ((result.tj_year % 4 == 0) && (result.tj_year % 100 != 0) || (result.tj_year % 400 == 0)) {//真，则为闰年)
		month[1] = 29;
	}
	while (result.tj_day > month[result.tj_month - 1]) {
		result.tj_day = result.tj_day - month[result.tj_month - 1];
		result.tj_month++;
		while (result.tj_month > 12) {
			...
			result.tj_month = result.tj_month - 12;
		}
	}
	/* 实现过程结束 */
